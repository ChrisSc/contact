...
- **`event-completeness.test.ts`** — Full game integration: plays complete game from construction to victory, then audits logger buffer for event completeness. Verifies: `game.start` first, 14 `fleet.place` events (7/player), `game.phase_change` transitions (SetupP1→SetupP2, SetupP2→Combat), `combat.fire`/`combat.hit`/`combat.miss` pairing, 7 `combat.sunk` events with remaining:0, `game.turn_start`/`game.turn_end` pairing (starts = ends + 1), `game.victory` last game-category event, monotonically increasing sequence numbers with no gaps, 2 `fleet.confirm` events, 22 `combat.hit` events, `economy.credit` event structure validation
...
- **Exhaustive firing**: Systematic firing through all 343 cells to confirm victory before exhaustion. Uses nested loop with fallback cell-finding for player 1.
...
- **G-SONAR tests**: Use `createTestPlayerState` with manually set cell states. Verify per-cell `rawResult`/`displayedResult`/`written` fields. No jammer interaction (distinct from drone). 49 cells per scan at specified depth.
