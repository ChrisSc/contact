...
| `components/perk-store.ts` | `PerkStore`: slide-out panel (desktop), full-screen overlay (mobile). Offensive (red) / Defensive (green). Cost badges amber. |
| `components/inventory-tray.ts` | `InventoryTray`: purchased perks grouped by type with count badges |
| `components/action-slots.ts` | `ActionSlots`: PING / ATTACK / DEFEND slot HUD. Shows checkmark when `slotUsed` is true (checked before `hasInventory`). |
...
| `screens/handoff-screen.ts` | Player transition with ready confirmation. Calls `initAudioContext()` on READY. |
| `screens/combat-screen.ts` | Canvas-dominant 3D. Fire/abilities via raycaster. Board toggle + action slots wrapped in `tab-strip` container. All perk modes, animations, audio, notifications. Ambient starts on mount. Rank bonus notification on mount, DRY counter in bottom bar for non-officer ranks. AI mode: auto-executes AI turns via `context.aiOpponent.executeTurn()`, locks UI during AI thinking. |
...
- **Tab strip**: `combat-screen__tab-strip` wraps board toggle + action slots. `display: contents` on desktop (pass-through), `display: flex` on mobile (scrollable row).
- **Persistent footer**: Created in `main.ts`, z-index 5. All bottom UI elements offset 20px to clear it. Hidden on mobile.
