...
- **Logger integration** — `audio.init` on context start, `audio.play` on each sound trigger (with sound name), `audio.mute` on toggle, `audio.phase_change` on game phase transitions.
...
- **No persistent state** (SFX): No synth pools or reusable nodes. Each SFX call creates fresh nodes to avoid timing conflicts from rapid re-triggers.
- **Persistent state** (ambient): `ambient.ts` maintains long-lived Tone.js nodes (oscillator, noise, filters, volumes) that run continuously during combat. `stopAmbient()` disposes all nodes.
...
Combat screen calls `initAudioContext()` on first user interaction (any click handler), then starts ambient if not running. Sound functions are called directly from UI handlers:
...
|-----------|-------|-----|------------|---------|
| Hum freq | 40 Hz | 35 Hz | 30 Hz | 25 Hz |
| Hum vol | -22 dB | -18 dB | -16 dB | -14 dB |
| Noise vol | -35 dB | -28 dB | -24 dB | -20 dB |
| Noise cutoff | 80 Hz | 120 Hz | 180 Hz | 250 Hz |
| Ping interval | 10s | 7s | 5s | 3s |

Phase transitions use 2-second `rampTo` for smooth parameter changes. Ping interval only reschedules when the value actually changes.
